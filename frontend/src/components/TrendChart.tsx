...
export function TrendChart({ data, isLoading: externalLoading, days = 7, onDaysChange, animate = true }: TrendChartProps) {
  ...
  const fetchTrendData = useCallback(async (numDays: number) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        const { dates, irrigation, trends, sources } = result.data
        const trendData: TrendDataPoint[] = dates.map((date: string, i: number) => ({
          date,
          irrigation: irrigation[i],
          trend: trends[i] || 'same',
          source: sources?.[i] === 'Override' ? 'Override' as const : 'TSMixer' as const,
        }))
  ...
  useEffect(() => {
    ...
    setDisplayIndex(animate ? 0 : data.length)
  }, [data, animate])

  useEffect(() => {
    if (!animate || sourceData.length === 0) return
    if (displayIndex >= sourceData.length) return
    animationRef.current = window.setTimeout(() => {
      setDisplayIndex(prev => Math.min(prev + 1, sourceData.length))
    }, 200)
    return () => {
      if (animationRef.current) clearTimeout(animationRef.current)
    }
  }, [displayIndex, sourceData.length, animate])
