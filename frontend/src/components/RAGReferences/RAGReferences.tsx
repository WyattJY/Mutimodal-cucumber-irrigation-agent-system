...
const isUploadedPdfReference = (ref: RAGReference) =>
  ref.source_type === 'user_upload' || Boolean(ref.page_url)

const referenceLabel = (ref: RAGReference) => {
  const source = ref.source || ref.title || 'FAO56'
  return `${source}${ref.page ? ` P${ref.page}` : ''}`
}
...
export const CompactRAGReferences: React.FC<CompactRAGReferencesProps> = ({
  ...
  const openReference = async (ref: RAGReference) => {
    if (isUploadedPdfReference(ref) && ref.page_url) {
      window.open(resolveBackendUrl(ref.page_url), '_blank', 'noopener,noreferrer')
      return
    }

    setSelectedRef(ref)
    setDetailText(ref.snippet || '')
    if (!ref.detail_url) return

    setIsLoadingDetail(true)
    try {
      const response = await fetch(resolveBackendUrl(ref.detail_url))
      const payload = await response.json()
      const detail = payload?.data?.content || payload?.data?.snippet || ref.snippet || ''
      setDetailText(detail)
    } catch {
      setDetailText(ref.snippet || '暂无可展开内容')
    } finally {
    ...
    <>
        ...
        <div className="rag-references__list">
          {displayRefs.map((ref, index) => (
            <button
              ...
              title={isUploadedPdfReference(ref) ? '打开 PDF 对应页' : '查看引用内容'}
              onClick={() => openReference(ref)}
            >
              <span className="rag-reference-item__index">[{index + 1}]</span>
              <span className="rag-reference-item__source">{ref.source || ref.title || 'FAO56'}</span>
              {ref.page && <span className="rag-reference-item__page">P{ref.page}</span>}
              <span className="rag-reference-item__action">
                {isUploadedPdfReference(ref) ? '打开' : '查看'}
              </span>
      ...
      {selectedRef && (
        <div className="reference-modal" role="dialog" aria-modal="true">
          <div className="reference-modal__backdrop" onClick={() => setSelectedRef(null)} />
          <div className="reference-modal__panel">
            <div className="reference-modal__header">
              <div>
                <h3>{referenceLabel(selectedRef)}</h3>
                <p>{selectedRef.source_type === 'fao56' ? 'FAO56 知识片段' : '知识库引用片段'}</p>
              </div>
              <button type="button" onClick={() => setSelectedRef(null)} aria-label="关闭引用详情">
                <i className="ph-bold ph-x"></i>
              </button>
            </div>
            <div className="reference-modal__body">
              {isLoadingDetail ? '正在加载引用内容...' : detailText}
            </div>
          </div>
        </div>
      )}
