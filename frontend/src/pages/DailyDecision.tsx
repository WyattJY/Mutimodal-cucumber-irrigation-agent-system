...
import { useLatestEpisode, useOverrideMutation } from '@/hooks/useEpisodes'
...
export function DailyDecision() {
  const { data: episode, isLoading, error, refetch: refetchLatestEpisode } = useLatestEpisode()
  ...
  const handleOverrideConfirm = async (amount: number, reason: string) => {
    if (!episode?.date) {
      throw new Error('No episode date available for override')
    }
    await overrideMutation.mutateAsync({
      date: episode.date,
      original_value: irrigationAmount,
      replaced_value: amount,
      reason,
      confirmation_answers: {
        source: 'daily_decision',
      },
    })
    await refetchLatestEpisode()
  }

  const recentAgentMessages = agentMessages.slice(-4)
        ...
        <div className="right-panel">
          ...
          <div className="glass-panel insights-card">
            ...
            {showImageCompare && episode?.date ? (
              ...
              {recentAgentMessages.map((message) => (
                <div className="insight-msg" key={message.id}>
                  <div className={`insight-msg__bubble ${message.role === 'assistant' ? 'insight-msg__bubble--forecast' : ''}`}>
                    <div className="insight-msg__header">
                      <span className={`insight-msg__tag ${message.role === 'assistant' ? 'insight-msg__tag--forecast' : 'insight-msg__tag--growth'}`}>
                        {message.role === 'assistant' ? 'Agent' : 'User'}
                      </span>
                      <span className="insight-msg__time">
            ...
            <div className="insights-card__typing">
              <span className="insights-card__typing-text">
                {isAgentStreaming ? 'Agri-Copilot responding' : 'Agri-Copilot ready'}
              </span>
              {isAgentStreaming && <span className="insights-card__typing-cursor"></span>}
            </div>
            ...
            <div className="insights-card__input">
              <div className="insights-card__input-wrapper">
                ...
                />
                <button
                  className="insights-card__input-btn"
                  onClick={handleAgentSubmit}
                  disabled={!agentQuestion.trim() || isAgentStreaming}
                >
                  <i className={`ph-bold ${isAgentStreaming ? 'ph-spinner' : 'ph-paper-plane-right'}`}></i>
                </button>
              </div>
            </div>
                  </>
