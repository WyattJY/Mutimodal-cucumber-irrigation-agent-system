...
const MOCK_MODELS: Record<Provider, string[]> = {
  ...
  custom: ['gpt-chat-latest', 'custom-model-v1', 'finetuned-agri-v2'],
}
...
export function Settings() {
  ...
  const [currentConfig, setCurrentConfig] = useState<LLMSettings | null>(null)

  // 加载当前配置
  useEffect(() => {
    void refreshCurrentConfig()
  }, [])
  ...
  const refreshCurrentConfig = async () => {
    try {
      ...
      setUseCustomConfig(data.use_custom_config)
      if (data.custom_base_url) {
        setApiUrl(data.custom_base_url)
      }
      if (data.custom_model) {
        setSelectedModel(data.custom_model)
      }
    } catch (err) {
              ...
              <div className="hybrid-card" style={{ marginBottom: '1rem' }}>
                <div className="hybrid-card__info">
                  ...
                  <div>
                    ...
                    <p className="hybrid-card__desc">关闭则使用系统默认 API ({currentConfig?.default_model || '系统默认模型'})</p>
                  </div>
                </div>
