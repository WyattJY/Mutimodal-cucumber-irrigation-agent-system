...
export function History() {
  ...
  const { data: episodesData, isLoading } = useEpisodesQuery({ page_size: 120 })
  const { data: weeklySummariesData, isLoading: isWeeklyLoading } = useWeeklySummaries()
  const {
    isStreaming: isTrendQueryStreaming,
    sendMessage: sendTrendQuery,
    setOpen: setChatOpen,
  } = useChatStore()
  const episodes: Episode[] = episodesData?.items || []
  const weeklySummaries = weeklySummariesData?.items || []
  ...
  const selectedWeeklySummary = weeklySummaries[selectedWeek] || null
  const selectedWeekEpisodes = weeklyData[selectedWeek]?.episodes || []
  const selectedWeeklyStats = selectedWeeklySummary ? {
    totalIrrigation: selectedWeeklySummary.stats.total_irrigation,
    avgIrrigation: selectedWeeklySummary.stats.avg_irrigation,
    betterDays: selectedWeeklySummary.stats.trend_distribution.better,
    sameDays: selectedWeeklySummary.stats.trend_distribution.same,
    worseDays: selectedWeeklySummary.stats.trend_distribution.worse,
    overrideCount: selectedWeeklySummary.stats.override_count,
  } : {
    totalIrrigation: selectedWeekEpisodes.reduce((sum, ep) => sum + (ep.irrigation_amount || 0), 0),
    avgIrrigation: selectedWeekEpisodes.length > 0
      ? selectedWeekEpisodes.reduce((sum, ep) => sum + (ep.irrigation_amount || 0), 0) / selectedWeekEpisodes.length
      : 0,
    betterDays: selectedWeekEpisodes.filter((ep) => ep.response?.trend === 'better').length,
    sameDays: selectedWeekEpisodes.filter((ep) => ep.response?.trend === 'same').length,
    worseDays: selectedWeekEpisodes.filter((ep) => ep.response?.trend === 'worse').length,
    overrideCount: selectedWeekEpisodes.filter((ep) => ep.decision_source === 'Override').length,
  }
  const selectedWeeklyInsights = selectedWeeklySummary?.insights?.length
    ? selectedWeeklySummary.insights.map((text) => ({ text, type: 'info' as const }))
    : insights
  const selectedWeeklyTrendData = selectedWeekEpisodes
    .slice()
    .reverse()
    .map((ep) => ({
      date: ep.date,
      irrigation: ep.irrigation_amount || 0,
      trend: ep.response?.trend || 'same',
      source: ep.decision_source === 'Override' ? 'Override' as const : 'TSMixer' as const,
    }))

  const handleTrendQuestionSubmit = async () => {
    const message = trendQuestion.trim()
    if (!message || isTrendQueryStreaming) return
    setTrendQuestion('')
    setChatOpen(true)
    await sendTrendQuery(`请基于当前灌溉历史数据回答：${message}`)
  }

  const handleTrendQuestionKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
      event.preventDefault()
      handleTrendQuestionSubmit()
    }
  }
      ...
      <div className="analytics-header">
        <div className="analytics-search">
          ...
          <input
            ...
            value={trendQuestion}
            onChange={(event) => setTrendQuestion(event.target.value)}
            onKeyDown={handleTrendQuestionKeyDown}
            disabled={isTrendQueryStreaming}
          />
          <button
            type="button"
            className="analytics-search__shortcut"
            onClick={handleTrendQuestionSubmit}
            disabled={!trendQuestion.trim() || isTrendQueryStreaming}
            title="发送趋势问题"
          >
            <i className={`ph-bold ${isTrendQueryStreaming ? 'ph-spinner' : 'ph-paper-plane-right'}`}></i>
          </button>
        </div>
            ...
            <div className="weekly-insights">
              ...
              <div className="analytics-card week-selector">
                ...
                <div className="week-selector__list">
                  {isWeeklyLoading ? (
                    <p style={{ color: 'var(--color-text-muted)', padding: '1rem' }}>正在加载周总结...</p>
                  ) : weeklySummaries.length > 0 ? weeklySummaries.map((summary, i) => (
                      <button
                      ...
                      >
                        ...
                        {summary.week_start} ~ {summary.week_end.slice(5)}
                      </button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div className="stats-row">
                  <div className="analytics-card stat-box">
                    ...
                    <p className="stat-box__value">
                      {selectedWeeklyStats.totalIrrigation.toFixed(1)} <span className="stat-box__unit">L/m²</span>
                    </p>
                  </div>
                  <div className="analytics-card stat-box">
                    <p className="stat-box__label">日均灌水</p>
                    <p className="stat-box__value">
                      {selectedWeeklyStats.avgIrrigation.toFixed(2)} <span className="stat-box__unit">L/m²</span>
                    </p>
                  </div>
                  <div className="analytics-card stat-box">
                    <p className="stat-box__label">长势评分</p>
                    <p className="stat-box__value stat-box__value--accent">
                      {selectedWeeklyStats.betterDays >= 5 ? 'A' : selectedWeeklyStats.betterDays >= 3 ? 'B+' : selectedWeeklyStats.betterDays >= 2 ? 'B' : 'C'}
                    </p>
                  </div>
                  <div className="analytics-card stat-box">
                    <p className="stat-box__label">人工覆盖</p>
                    <p className="stat-box__value">
                      {selectedWeeklyStats.overrideCount} <span className="stat-box__unit">次</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="analytics-card" style={{ padding: '1.5rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {selectedWeeklyInsights.map((insight, idx) => (
                      <div
                        key={idx}
                        style={{
                          padding: '1rem',
