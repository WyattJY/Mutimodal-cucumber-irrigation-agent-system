...
class JsonKnowledgeStore:
    ...
    def __init__(self, json_path: Optional[str] = None, user_literature_dir: Optional[str] = None):
        """
        初始化知识库
        ...
        if user_literature_dir is None:
            base_dir = Path(__file__).parent.parent.parent.parent
            user_literature_dir = base_dir / "data" / "user_literature"
        self.user_literature_dir = Path(user_literature_dir)
    ...
    def _load_chunk_file(self, path: Path) -> int:
        ...
        for item in items:
            ...
            chunk = KnowledgeChunk(
                unique_id=item.get("unique_id") or item.get("doc_id") or f"{path.stem}_{loaded}",
                content=item.get("page_content") or item.get("content") or item.get("snippet", ""),
                page_num=int(item.get("page_num") or item.get("page") or metadata.get("page") or 0),
                file_name=item.get("file_name") or item.get("source") or metadata.get("source") or path.name,
                metadata=metadata,
            )
            if chunk.content:
    ...
    def _ensure_loaded(self):
        ...
        try:
            if self.json_path.exists():
                self._load_chunk_file(self.json_path)
            else:
