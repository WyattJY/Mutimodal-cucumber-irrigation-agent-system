...
class YoloMetrics(BaseModel):
    """YOLO 检测指标"""
    leaf_instance_count: float = 0.0
    leaf_average_mask: float = 0.0
    flower_instance_count: float = 0.0
    flower_mask_pixel_count: float = 0.0
    terminal_average_mask_pixel_count: float = 0.0
    fruit_mask_average: float = 0.0
    all_leaf_mask: float = 0.0
    leaf_area: float = 0.0
...
class Abnormalities(BaseModel):
    """异常情况"""
    wilting: Literal["none", "mild", "moderate", "severe"] = "none"
    yellowing: Literal["none", "mild", "moderate", "severe"] = "none"
    pest_damage: Literal["none", "mild", "moderate", "severe"] = "none"
    pests: Literal["none", "mild", "moderate", "severe"] = "none"
    disease: Literal["none", "mild", "moderate", "severe"] = "none"
    other: str = ""


class Comparison(BaseModel):
    """变化对比"""
    leaf_area_change: Any = ""
    leaf_count_change: Any = ""
    flower_count_change: Any = ""
    fruit_count_change: Any = ""
    overall_vigor_change: Any = ""
    overall_health_change: Any = ""
...
class PredictOptions(BaseModel):
    ...
    skip_tsmixer: bool = False
    skip_plant_response: bool = False
...
class GraphNodeTrace(BaseModel):
    ...
    status: str = Field(..., description="执行状态 (ok/skipped/cached/error/fallback/warn)")
    detail: Optional[str] = Field(None, description="详细信息")
    value: Optional[float] = Field(None, description="节点输出值")
    trend: Optional[str] = Field(None, description="长势趋势")
    has_anomaly: Optional[bool] = Field(None, description="是否有异常")
    severity: Optional[str] = Field(None, description="异常严重程度")
    consistent: Optional[bool] = Field(None, description="SanityCheck 一致性")
    adjusted_value: Optional[float] = Field(None, description="调整后的值")
    iteration: Optional[int] = Field(None, description="迭代次数")
    count: Optional[int] = Field(None, description="检索数量")
    cold_start: Optional[bool] = Field(None, description="是否冷启动")
    source: Optional[str] = Field(None, description="预测来源")


class GraphPredictResult(BaseModel):
    """LangGraph Pipeline 预测结果 (v2)"""
    date: str = Field(..., description="日期")
    irrigation_amount: float = Field(..., description="预测灌水量 L/m²")
    source: str = Field(..., description="预测来源")
    is_cold_start: bool = Field(False, description="是否为冷启动")

    yolo_metrics: Optional[dict] = Field(None, description="YOLO 指标")
    plant_response: Optional[dict] = Field(None, description="长势评估结果")
    sanity_check: Optional[dict] = Field(None, description="合理性复核结果")
    rag_references: List[dict] = Field(default_factory=list, description="RAG 检索引用")
    ...
    graph_trace: List[GraphNodeTrace] = Field(default_factory=list, description="图执行追踪")
    graph_version: str = Field("v2", description="Pipeline 版本")
...
class WeeklyStatistics(BaseModel):
    """周统计数据"""
    total_irrigation: float = Field(0.0, description="总灌水量")
