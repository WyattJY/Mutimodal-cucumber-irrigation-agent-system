...
class RetrievalResult:
    ...
    def page_content(self) -> str:
        """LangChain-compatible document text property."""
        return self.snippet
...
class RAGService:
    ...
    def _load_chunk_json_file(self, json_file: Path):
        ...
        for idx, item in enumerate(data if isinstance(data, list) else [data]):
            ...
            content = item.get("content") or item.get("page_content") or item.get("snippet") or ""
            if not content:
                continue
            source = item.get("file_name") or item.get("source") or metadata.get("source") or json_file.name
            self._documents.append({
                "doc_id": item.get("doc_id") or item.get("unique_id") or f"{json_file.stem}_{idx}",
                "title": item.get("title") or source,
                "content": content,
                "metadata": {
                    **metadata,
                    "source": source,
                    "page": item.get("page_num") or item.get("page") or metadata.get("page"),
                    "chunk_file": str(json_file),
                },
            })

    def reload(self):
        """Reload local RAG files after user literature upload."""
        self._documents = []
        self._initialized = False
        self._load_knowledge_base()
    ...
    def _load_knowledge_base(self):
        ...
        try:
            ...
            fao_chunks = project_root / "data" / "knowledge_base" / "fao56_chunks.json"
            if fao_chunks.exists():
                self._load_chunk_json_file(fao_chunks)
            ...
            if user_literature_dir.exists():
                for chunk_file in sorted(user_literature_dir.glob("*_chunks.json")):
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to load user literature chunks {chunk_file}: {exc}")
    ...
    async def retrieve(
        ...
        """
        Agentic RAG entrypoint used by LangGraph subagents.

        Retrieval order:
        1. Redis cache lookup.
        2. ChromaDB vector query when available.
        3. Local BM25-like keyword fallback over JSON/Markdown files.

        The return contract intentionally mirrors LangChain Document enough for
        downstream nodes: each result exposes ``page_content`` and ``metadata``.
        """
        cache_key = "rag:" + hashlib.sha256(
            json.dumps({"q": query, "k": top_k, "f": filters}, sort_keys=True, ensure_ascii=False).encode("utf-8")
        ).hexdigest()
        ...
        try:
            ...
            if cached:
                return [RetrievalResult(**row) for row in cached]
        except Exception:
            cached = None

        chroma_rows: list[dict] = []
        try:
            from app.infra.chroma_store import chroma_manager

            chroma_rows = chroma_manager.query(query, top_k=top_k, where=filters)
        except Exception:
            chroma_rows = []
        ...
        if chroma_rows:
            results = [
                RetrievalResult(
                    doc_id=row.get("doc_id", f"chroma_{idx}"),
                    title=row.get("title"),
                    snippet=row.get("snippet", ""),
                    relevance=float(row.get("relevance", 0.0)),
                    metadata=row.get("metadata") or {"retrieval_backend": "chroma"},
                )
                for idx, row in enumerate(chroma_rows)
            ]
        else:
            results = await self.search(query=query, top_k=top_k, filters=filters)
            for result in results:
                metadata = result.metadata or {}
                metadata.setdefault("retrieval_backend", "keyword_bm25_fallback")
                metadata.setdefault("chunk_id", result.doc_id)
