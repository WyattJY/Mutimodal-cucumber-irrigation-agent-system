...
请结合YOLO实例分割结果，提供专业的农艺建议。"""
    ...
    @property
    def model(self) -> str:
        return get_active_openai_config().get("model") or "unknown"
    ...
    async def test_connection(self) -> dict:
        try:
            ...
            response = await client.chat.completions.create(
                ...
                **completion_token_kwargs(model, 5),
            )
            return {"success": True, "message": "连接成功", "model": model, "model_chat": model, "model_vision": vision_model, "response": response.choices[0].message.content}
        except Exception as e:
            model = self.model
            return {"success": False, "message": f"连接失败: {str(e)}", "model": model, "model_chat": model, "model_vision": self.vision_model}
    ...
    async def sanity_check(
        ...
        try:
            response = await client.chat.completions.create(
                ...
                response_format={"type": "json_object"},
                **temperature_kwargs(model, 0.3),
                **completion_token_kwargs(model, 1024),
            )
            response_text = response.choices[0].message.content
            data = json.loads(response_text)
