...
from .attention import DAttention, SEAttention
...
class Faster_Block_CPCA(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
        ...
        self.attention = CPCA(channels=dim)
...
class Bottleneck_FBC(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, n_div=4, mlp_ratio=2, drop_path=0.1,
        ...
        self.cv2 = Conv(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2
        self.faster_block_cpca = Faster_Block_CPCA(c2, c2, n_div, mlp_ratio, drop_path, layer_scale_init_value,
                                                   pconv_fw_type)

    def forward(self, x):
        out = self.cv1(x)
        out = self.cv2(out)
        out = self.faster_block_cpca(out)
        return x + self.faster_block_cpca(self.cv2(self.cv1(x))) if self.add else out
...
class C3_FBC(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_FBC(c_, c_) for _ in range(n)))
...
class C3k2_FBC(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3_FBC(self.c, self.c, 2, shortcut) if c3k else Bottleneck_FBC(self.c, self.c) for _
            in range(n)
        )
########################################################################
...
class MANet(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, p=1, kernel_size=3, g=1, e=0.5):
        ...
        # self.cv_first = GSConvns(c1, 2 * self.c, 1, 1)
