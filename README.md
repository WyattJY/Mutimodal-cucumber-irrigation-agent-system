...
本目录是从 `G:\master_tools\projects\cucumber-irrigation-linux` 复制到 `G:\AI_TRAVEL\cucumber-irrigation-linux` 后的改造版。业务仍然是 **基于多模态融合的温室黄瓜灌水决策智能体研究**，不是旅游规划；`AI_TRAVEL` 指复用该项目的 Agent 工程栈与设计方法。

核心新增入口：

- `backend/app/graph/builder_v2.py`: LangGraph Router + Handoff + Send API Subagent 主图。
- `backend/app/api/v1/agent.py`: `/api/agent/decision` 与 `/api/agent/decision/stream`。
- `backend/app/graph/feature_builder.py`: 3 环境 + 7 YOLO + 1 历史灌水，共 11 维，96 天 TSMixer 窗口。
- `backend/app/infra/`: PostgreSQL、Redis、ChromaDB 可选连接与本地降级。
- `AI_TRAVEL_STACK_COVERAGE.md`: 技术栈与简历覆盖矩阵。

本地验证：

```bash
python -m pytest backend/tests/test_ai_travel_stack.py -q
```
